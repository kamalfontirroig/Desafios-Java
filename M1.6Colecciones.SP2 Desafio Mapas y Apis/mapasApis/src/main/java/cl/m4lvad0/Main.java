...
public class Main {
  ...
  public static void nombresmails() {
    ...
    List<User> listaUsuarios = respuestaUsuarios.readEntity(new GenericType<List<User>>(){});
    for (User datosUsuarios: listaUsuarios)
          System.out.println(datosUsuarios.getName()+" - "+datosUsuarios.getEmail());
}
public static void albumsDelUsuario(String idDelUsuario) {
    Client client = ClientBuilder.newClient();
    ...
    for (Album datosAlbums: listaAlbums)
        if(datosAlbums.getUserId().equals(idDelUsuario))
          System.out.println(datosAlbums.getId()+" - "+datosAlbums.getUserId()+" - "+datosAlbums.getTitle());
}

public static void detalleAlbum(int idAlbum) {
    Client client = ClientBuilder.newClient();
    WebTarget target = client.target("https://jsonplaceholder.typicode.com/albums/").path(Integer.toString(idAlbum));
    Invocation.Builder invocationBuilder = target.request(MediaType.APPLICATION_JSON);
    Response respuestaAlbums = invocationBuilder.get();
...
public static void fotosContienen(String contenidoEnTitulo) {
    Client client = ClientBuilder.newClient();
    ...
    List<Photo> listaFotos = respuestaFotos.readEntity(new GenericType<List<Photo>>(){});
    List<String> fotosOrden = new ArrayList<String>();
    for (Photo datosFotos: listaFotos)
        if(datosFotos.getTitle().contains(contenidoEnTitulo)) {
...
public static void usuariosAlbums2() {
 Client clientU = ClientBuilder.newClient();
 WebTarget targetU = clientU.target("https://jsonplaceholder.typicode.com/").path("users");
 Invocation.Builder invocationBuilderU = targetU.request(MediaType.APPLICATION_JSON);
 Response respuestaUsuarios = invocationBuilderU.get();
